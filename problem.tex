\section{The Cache Coherency Problem}
...
The aggregation of such coordination and rules are called \emph{cache coherency protocols}, which can be roughly categorized into three classes:
snooping, directory-based, and software approaches.

Snooping or snoopy coherency protocols~\cite{tang:csdittcms,goodman:ucmtrpt,katz:iaccp,em:dragon,thacker:firefly} are the most popular protocols in bus-sharing multiprocessor systems.
...
Directory-based protocols~\cite{agarwal:aeodsfcc,tang:csdittcms,patel:aomwpcm} use a directory to keep track of all cache blocks' status, including the block state and the set of processors that are sharing the block.
Compared with snooping approaches, directory-based protocols allow more fine-grained transaction monitoring instead of broadcasting each transaction to all processors connected to the bus.
These protocols achieve better scalability by reducing the bus traffic, but inevitably incur extra overhead for ensuring the atomicity and ordering of transaction signals.

Software-based protocols~\cite{smith:cccwssauoti,cytron:amopc} trade software complexity for hardware complexity.
They are highly extensible and configurable, requiring minimal hardware support.
Nevertheless, software-based protocols feel hard to achieve comparable performance to hardware-based competitors~\cite{owicki:etposcc}.

This notes focuses on snoopy coherency protocols, as they are the most widely used solutions.
Directory-based techniques are mainly applied in NUMA systems or large-scale multiprocessor systems because of their ideal scalability;
software-based approaches appear more often in special scenarios with limited hardware support or irregular requirements.